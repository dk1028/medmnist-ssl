...
- **Class differentiability:** Low resolution of the images (28x28) make them seem blurry and makes it hard to distinguish differences/patterns by human eye but images labeled "normal" (0) seem to generally have clearer, darker borders around the lungs while images labeled "pneumonia" (1) seem to
have a larger white patch in the center area (where the lung would be).
...
- The ECE is consistently ~0.25 which pretty high (so model confidence and accuracy differs significantly).
...
- Most of the mistakes are normal x-rays that get called pneumonia, which is expected from the class imbalance and overinflated accuracy score (model exploitation).
- Confidence is extremely high when making wrong predictions ~0.99.
- High brightness with hardly noticeable borderlines around the lung (similar to the general pattern you notice in pneumonia labeled images by human eye). 
