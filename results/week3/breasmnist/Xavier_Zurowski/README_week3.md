...
* **Head_only:** Train loss decreases (~0.65 → ~0.55), but validation loss stays higher and fluctuates, indicating limited generalization. Train accuracy rises slightly (~0.72 → ~0.74) while validation accuracy is flat at ~0.73, suggesting the head-only model saturates early and is capacity-limited/underfitting.
* **Finetune_all:** Train loss drops rapidly to near 0 and train accuracy reaches ~1 by epoch 8. Validation loss spikes severely early at epoch 2 before collapsing, while validation accuracy peaks at ~0.94. Overall suggesting minor overfitting but significantly better validation performance than head-only training.
* **AugA:** Train loss decreases steadily, and after early instability (validation loss spike at epoch 2), validation loss closely tracks train loss from ~epoch 5 onward. Train accuracy rises rapidly ~0.94, while validation accuracy increases to ~0.85 with a significant gap. This indicates mild overfitting but significantly better validation performance than head-only training.
